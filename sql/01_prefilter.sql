...
WITH source AS (
  ...
  SELECT *,
    CASE
      ...
      WHEN regexp_matches(normalized_path, '/app/rest/') THEN 'JetBrains TeamCity'
      ...
      WHEN regexp_matches(normalized_path, '/actuator/') THEN 'Spring Boot Actuator'
      ...
      WHEN regexp_matches(normalized_path, '/api/v4/') THEN 'GitLab'
      WHEN regexp_matches(normalized_path, '/wd/hub') THEN 'Selenium Grid'
      WHEN regexp_matches(normalized_path, '/terminals/') THEN 'Jupyter'
      WHEN regexp_matches(normalized_path, '/_cat/') THEN 'Elasticsearch'
      WHEN regexp_matches(normalized_path, '_all_dbs') THEN 'CouchDB'
    ...
    CASE
      ...
      WHEN regexp_matches(normalized_path, '(jquery|pdf(\.min)?\.js|angular|lodash|bootstrap|purify|ckeditor|tinymce)') THEN 'CLIENT_COMPONENT_PATH_OBSERVED'
  ...
  WHERE regexp_matches(normalized_path,
    '(\.env|/\.git/|\.map$|\.(sql|dump|sqlite|bak|tfstate|tfvars)|wp-config|settings\.php|id_rsa|id_ed25519|credentials|actuator|app/rest|geoserver|jolokia|global-protect|mgmt/tm|setupadministrator|setup/setup-|oauth/idp|logon/logonpoint|manager/(html|text)|jmx-console|web-console|invoker|wls-wsat|_ignition|eval-stdin|autodiscover|/ews/|/ecp/|vropspluginui|remote/fgt_lang|sslvpn|/cacti/|/cfide/|service/soap|goanywhere|moveitisapi|webinterface|/mifs/|/minio/|/nacos/|/apisix/|/druid/|sdk/weblanguage|validate/code|langflow|nifi|airflow|grafana|kibana|swagger|openapi|graphql|graphiql|phpinfo|server-status|api/v4|wd/hub|terminals|_cat|_all_dbs|v[12]/keys|jquery|pdf(\.min)?\.js|angular|lodash|bootstrap|purify|ckeditor|tinymce)')
